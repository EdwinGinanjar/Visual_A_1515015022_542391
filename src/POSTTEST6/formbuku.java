...
public class formbuku extends javax.swing.JFrame {
    ...
    //sintak diatas untuk menghubungkan program form dengan database yang akan digunakan.
    ...
    private void UbahData( String id, String judul, String penulis, String harga){ // sintak untuk melakukan perubahan pada data
        try{
    ...
    private void HapusData( String id, int baris){ // sintak untuk menghapus data dari tabel
        try{
            String sqldelete = "DELETE from buku where id="+id+""; // query menghapus data
            stt = con.createStatement();
    ...
    private void CariId(){ //method pencarian dengan merujuk pada id
        try{
            String sql = "SELECT*from buku where id='"+txtcari.getText()+"'"; //query pencarian data berdasarkan id
            stt = con.createStatement();
            rss = stt.executeQuery(sql);
            while(rss.next()){
              Object[] o = new Object[4];   
    ...
    private void CariJudul(){ //method pencarian dengan merujuk pada judul
        try{
            String sql = "SELECT*from buku where judul='"+txtcari.getText()+"'"; //query pencarian data berdasarkan judul buku
            stt = con.createStatement();
            rss = stt.executeQuery(sql);
            while(rss.next()){
              Object[] o = new Object[4];   
    ...
    private void CariPenulis(){ //method pencarian dengan merujuk pada penulis
        try{
            String sql = "SELECT*from buku where penulis='"+txtcari.getText()+"'"; //query pencarian data berdasarkan penulis buku
            stt = con.createStatement();
            rss = stt.executeQuery(sql);
            while(rss.next()){
              Object[] o = new Object[4];   
    ...
    private void CariHarga(){ //method pencarian dengan merujuk pada harga
        try{
            String sql = "SELECT*from buku where harga='"+txtcari.getText()+"'"; //query pencarian data berdasarkan harga buku
            stt = con.createStatement();
            rss = stt.executeQuery(sql);
            while(rss.next()){
              Object[] o = new Object[4];   
...
//        model.removeRow(baris);
    ...
    private void goActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_goActionPerformed
        ...
        if(txtcari.getText().equals("")){
            ...
            model.fireTableDataChanged(); //pencarian memuat berdasarkan Id, Judul, Penulis, dan Harga
            if(txtby.getSelectedItem().equals("Id")){
             CariId();  
            }
            else if(txtby.getSelectedItem().equals("Judul")){
             CariJudul();
            }
            else if(txtby.getSelectedItem().equals("Penulis")){
             CariPenulis();
            }
            else if(txtby.getSelectedItem().equals("Harga")){
             CariHarga();
             }
        }
